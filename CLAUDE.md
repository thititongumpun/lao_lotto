...
- `news_post.py --kind hot` — /api/hot story → Gemini rewrite → **photo poster** (`build_poster`: 1minhotspot article page → source link → og:image; one Gemini vision call picks `portrait`/`scene` + 3 headline lines; `poster.py` renders 1080² — portrait = rembg cutout over a flux-2-klein-4b background, scene = graded photo; source photo with baked-in headline → no poster) → `/photos` post, or the old text post if any poster step fails. `--dry-run` leaves `/tmp/poster_<id>.jpg`.
